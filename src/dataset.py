...
from src.utils import get_affine_transform, affine_transform
from src.utils import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
from src.utils import flip, draw_dense_reg
...
class DSet(Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.edges = [[0, 1], [0, 2], [1, 3], [2, 4], [4, 6], [3, 5], [5, 6], [5, 7]]
        self.acc_idxs = [1, 2, 3, 4, 5, 6, 7, 8]
        self.data_dir = opt.data_dir
        self.img_dir= opt.image_dir
        self.calib_dir = opt.calib_dir
        if split == 'test':
            self.annot_path = opt.working_dir + 'ann_test.json'
        elif split == 'val':
            self.annot_path = opt.working_dir + 'ann_val.json'
        elif split == 'train':
            self.annot_path = opt.working_dir + 'ann_train.json'
        self.max_objs = 100#32
    ...
    def __getitem__(self, index):
        ...
        img_path = self.img_dir + str(file_name) #os.path.join(self.img_dir, file_name)
        ...
        dense_kps = np.zeros((num_joints, 2, self.opt.output_h, self.opt.output_w), dtype=np.float32)
        dense_kps_mask = np.zeros((num_joints, self.opt.output_h, self.opt.output_w), dtype=np.float32)
        ...
        for k in range(num_objs):
            ...
            if (h > 0 and w > 0) or (rot != 0):
                ...
                for j in range(num_joints):
                    ...
                    if pts[j, 2] > 0:
                        if pts[j, 0] >= 0 and pts[j, 0] < self.opt.output_w and pts[j, 1] >= 0 and pts[j, 1] < self.opt.output_h:
