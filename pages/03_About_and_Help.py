...
with st.expander(st.header('About'),expanded=True):
...
with st.expander(st.header('Installation')):
    st.markdown(md_install, unsafe_allow_html=True)

with st.expander(st.header('Help: Main Page for Training and Inference')):
    st.warning("""
...
with st.expander(st.header('Help: Calibrate Camera Page')):
    st.write("""
...
with st.expander(st.header('Help: Label Dataset Page')):
    st.write("""
